...
void PrintTask_ex26(void* pvParameters);

/* Protótipo da função para imprimir no terminal */
void PrintString_ex26(const char* pcString);

void main_ex26(void)
{
	/*Cria a Task Printer 1 */
	xTaskCreate(PrintTask_ex26, "Print1", 1000, "1", 1, NULL);
	printf("Task Printer 1 Criada!\r\n\n");

	/*Cria a Task Printer 2 */
	xTaskCreate(PrintTask_ex26, "Print2", 1000, "2", 1, NULL);
...
static void PrintTask_ex26(void* pvParameters)
{
	char* StringToPrint;
	...
	for (;; )
	{
		...
		PrintString_ex26(StringToPrint);

		vTaskDelay(2);
	}
}

static void PrintString_ex26(const char* pcString)
{
	unsigned int ui;
	unsigned long ul;
